...
#### Redux를 활용한 React Component & Web Component (via StencilJS) 상태 연동
...
- Redux를 활용해 React Component와 StencilJS Web Component의 상태 연동하여 상품 추천 팝업 배너 디자인 수정 페이지 구현
  - OOP에 기반해 StencilJS Web Component의 style을 관리하는 `Banner Manager` 객체 작성
    - `structedClone`을 활용해 상태 업데이트 반영
  - React, `@reduxjs/toolkit`을 활용한 `FLUX` 패턴을 기반으로 디자인 수정 페이지 구현
