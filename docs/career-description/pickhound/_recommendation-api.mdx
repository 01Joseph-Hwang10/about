...
- => 디버깅 과정을 통해 모노리포 상황이나 `npm` 패키지 퍼블리싱 상황에서 패키지 간의 호환성 이슈를 상기해야함을 머릿 속에 각인함.
...
- 도큐먼트 읽기 횟수 `30,000,000/일 -> 1,500,000/일`로 감소
  - => 매우 큰 비용 절감 효과
...
- => Cloud Build, GCE 인스턴스 그룹, GCLB와 구글 관리형 SSL 등 Cloud Computing에 관한 이해도와 Docker, Bash script 등 DevOps 기술에 관한 이해도 크게 향상.
