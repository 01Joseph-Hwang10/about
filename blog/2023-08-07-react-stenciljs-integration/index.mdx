...
authors: joseph
tags: [React, Redux, StencilJS, OOP, FLUX, "@shepherd23"]
---
...
개인화 상품 추천 플러그인 [PickHound](https://store.cafe24.com/kr/apps/17367)의 개발 부문을 담당한 바가 있습니다.
...
처음에는 이를 Redux로 해결하고자 했습니다. `@reduxjs/toolkit`의 `createSlice` API를 이용해 `DesignSlice`를 만들고, 여기에서 추천 배너의 모든 디자인 정보를 관리하고, 배너의 디자인 정보를 변경할 때마다 `DesignSlice`의 상태를 업데이트하도록 하였습니다.

그러나 변경 가능한 디자인 옵션이 여러가지인 상황에서 모든 코드를 `DesignSlice`에 몰아넣는 것은 좋은 방법이 아니라고 생각했습니다.
또한 React의 상태와 StencilJS로 빌드된 Web Component의 상태를 한 곳에서 관리하는 것도 좋은 방법이 아니라고 생각했습니다.

## Banner Manager

이에 StencilJS로 빌드한 Web Component의 style을 관리하는 별도의 객체를 작성하고 DesignSlice는 React 단의 상태만 관리하게끔 만들고자 하였습니다.
...
export const DefaultBannerDesignSlice = createSlice({
  ...
  reducers: {
    setTitle: (state, action: PayloadAction<State["title"]>) => {
      ...
      if (banner.interactive && state.title.length !== 0) {
...
개발하고자 하는 기능이 다소 복잡했지만, OOP와 모듈화에 대한 계속적인 고민을 통해 코드의 Readability와 유지보수의 용이함을 보장할 수 있었습니다. 또한 작업을 진행하면서 코드 패턴에 관한 깊은 고찰과 Javascript, React, Redux, Web Component 의 동작 원리에 대한 이해를 크게 높일 수 있었습니다.
